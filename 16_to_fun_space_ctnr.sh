...
mskFiles=( $(ls | grep sub-12_ses-002_ctnr_ | grep .nii.gz) )
numMsk=${#mskFiles[@]}
numMsk=$(($numMsk - 1))
mskFiles_lh=( $(ls | grep sub-12_ses-002_lh_ctnr_ | grep .nii.gz) )
numMsk_lh=${#mskFiles_lh[@]}
numMsk_lh=$(($numMsk_lh - 1))
mskFiles_rh=( $(ls | grep sub-12_ses-002_rh_ctnr_ | grep .nii.gz) )
numMsk_rh=${#mskFiles_rh[@]}
numMsk_rh=$(($numMsk_rh - 1))
echo ${numMsk}
# -----------------------------------------------------------------------------
cd ${strPathOrig}
...
for i in $(seq 0 ${numMsk_lh});
do

  interp01=$(cut -d '.' -f 1 <<< ${mskFiles_lh[${i}]})


  $ANTSPATH/antsApplyTransforms -d 3 \
  -i ${mskPth}${mskFiles_lh[${i}]} \
  -o ${mskPth}fun_space/LefRight/${interp01}_reg2run.nii.gz \
  -n GenericLabel \
  -r ${moving_fun} \
  -t [${trfPth}sub-12_ses-002_fun_to_anat_0GenericAffine.mat,1] \
  -v
...
for i in $(seq 0 ${numMsk_rh});
do

  interp01=$(cut -d '.' -f 1 <<< ${mskFiles_rh[${i}]})


  $ANTSPATH/antsApplyTransforms -d 3 \
  -i ${mskPth}${mskFiles_rh[${i}]} \
  -o ${mskPth}fun_space/LefRight/${interp01}_reg2run.nii.gz \
  -n GenericLabel \
  -r ${moving_fun} \
  -t [${trfPth}sub-12_ses-002_fun_to_anat_0GenericAffine.mat,1] \

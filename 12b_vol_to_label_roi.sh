...
pRF_oc_ROI=( $(ls | grep pRF_results_ovrlp_mask_${olvp_prct[${olvpid}]}prct | grep .nii.gz) )
numROI=${#pRF_oc_ROI[@]}
numROI=$(($numROI - 1))
echo ${numROI}
# -----------------------------------------------------------------------------
...
cd ${volPth}rh
# Get all the pRF overlay results
pRF_oc_ROI_rh=( $(ls | grep pRF_results_ovrlp_mask_${olvp_prct[${olvpid}]}prct | grep .nii.gz) )
# -----------------------------------------------------------------------------
cd ${strPathOrig}
...
mri_cor2label \
--i ${volPth}stimulus_roi_comb_reg2fsanat_msk_lh.nii.gz \
--surf ${subjID} lh inflated \
--l ${labelPth}stimulus_roi_comb_reg2fsanat_msk_lh.label \
--id 1

mri_cor2label \
--i ${volPth}rh/stimulus_roi_comb_reg2fsanat_msk_rh.nii.gz \
--surf ${subjID} rh inflated \
--l ${labelPth}stimulus_roi_comb_reg2fsanat_msk_rh.label \
--id 1

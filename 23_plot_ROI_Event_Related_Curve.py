...
os.chdir(TxtPthIn)
NameLst = ['center', 'edge', 'background']
sns.set_style("white") 
# calculate for v1
namelabel_v1=[]
timepoint_v1=[]
...
for roi in range(len(NameLst)):
    v1_TxtFiles=glob.glob('v1_'+ NameLst[roi] + '*_meants.txt')
    for textid in range(len(v1_TxtFiles)):
        ...
        interp =  NameLst[roi] + '_' + v1_TxtFiles[textid].split('_')[2] 
...
v2_temp=[]
for roi in range(len(NameLst)):
    glob.glob('v2_'+ NameLst[roi] + '*_meants.txt')
...
namelabel_v3=[]
for roi in range(len(NameLst)):
    v3_TxtFiles=glob.glob('v3_'+ NameLst[roi] + '*_meants.txt')

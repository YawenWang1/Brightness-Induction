...
pRF_oc_ROI=( $(ls | grep pRF_results_ovrlp_mask_50prct | grep .nii.gz) )
numROI=${#pRF_oc_ROI[@]}
numROI=$(($numROI - 1))
echo ${numROI}
# -----------------------------------------------------------------------------
...
cd ${volPth}rh
# Get all the pRF overlay results
pRF_oc_ROI_rh=( $(ls | grep pRF_results_ovrlp_mask_50prct | grep .nii.gz) )
# -----------------------------------------------------------------------------
cd ${strPathOrig}
...
mri_cor2label \
--i ${volPth}stimuli_roi_50prct_ori_reg2fsanat_lh.nii.gz \
--surf ${subjID} lh inflated \
--l ${labelPth}stimuli_roi_50prct_ori_reg2fsanat_lh.label \
--id 1

mri_cor2label \
--i ${volPth}stimuli_roi_50prct_ori_reg2fsanat_lh.nii.gz \
--surf ${subjID} lh sphere \
--l ${labelPth}stimuli_roi_50prct_ori_reg2fsanat_lh.sphere.label \
--id 1

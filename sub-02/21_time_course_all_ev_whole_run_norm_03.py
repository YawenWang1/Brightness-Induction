...
meantsPth='/media/h/P04/Data/BIDS/sub-02/ses-002/func/GLM/BI/fslmeants'
os.chdir(meantsPth)
...
strPathOut = '/media/h/P04/Data/BIDS/sub-02/ses-002/func/GLM/BI/ERA_06'
os.chdir(strPathOut)
...
#     varBOLD_perc_down_01_mne = np.zeros([BaseDur,6])
     ...
     for index_02 in range(0, varNumIn_02):
          ...
          varCCond = 'fun_0' + str((index_02+1)) + '_' +  \
                    stiROI[index_01]  + '_meants_tc.txt'
...
#               # Calculate baseline before every epoch
          ...
          # To see whether there is nan in the array
          numNans =  np.unique(np.argwhere(np.isnan(varCCond_tc_norm))[:,1])
          if len(numNans) != 0:
               varCCond_tc_norm = np.delete(varCCond_tc_norm,numNans,1)
...
#               # Calculate baseline before every epoch
               ...
               var_up_perc[:,index_03] = var_up[:,index_03] - tmpBse_up
               var_down_perc[:,index_03]= var_down[:,index_03] -tmpBse_down
               
          varBOLD_perc_up[:,index_02*varTmpNumBlck:(index_02+1)*varTmpNumBlck]  =    var_up_perc
#               varBOLD_perc_up_01[:,index_02*varTmpNumBlck:(index_02+1)*varTmpNumBlck]  =    varBse_up_perc_01
#               
          varBOLD_perc_down[:,index_02*varTmpNumBlck:(index_02+1)*varTmpNumBlck]  =    var_down_perc
#               varBOLD_perc_down_01[:,index_02*varTmpNumBlck:(index_02+1)*varTmpNumBlck]  =    varBse_down_perc_01
          ...
          counter +=1
          plt.figure(counter)
          plt.plot(xaxis,np.mean(var_up_perc,axis=1))
#          plt.bar(2,4,width=5,aligh='center',color='grey')
          ...
          counter +=1
          plt.figure(counter)
          plt.plot(xaxis,np.mean(var_down_perc,axis=1))
          ...
          varBOLD_perc_up_mne[:,index_02]  =    np.mean(var_up_perc,axis=1)
          varBOLD_perc_down_mne[:,index_02]  =    np.mean(var_down_perc,axis=1)
...
#     plt.bar(2,4,width=5,aligh='center',color='grey')
     ...
     counter +=1
     plt.figure(counter)
     plt.plot(xaxis,np.mean(varBOLD_perc_up_mne,axis=1))
#     plt.bar(2,4,width=5,aligh='center',color='grey')
     plt.hlines(0,0,5,linestyles="dashed")
     plt.xticks(ind)
...
#     
     ...
     counter +=1
     plt.figure(counter)
     plt.plot(xaxis,np.mean(varBOLD_perc_down_mne,axis=1))
#     plt.bar(2,4,width=5,aligh='center',color='grey')
     plt.hlines(0,0,5,linestyles="dashed")
     plt.xticks(ind)
     plt.xlabel('Time/volume, TR = 2.604')
...
plt.figure(counter)
plt.hlines(0,0,5,linestyles="dashed")
yerr1 = stats.sem(aryBold[5],axis=1)
yerr2 = stats.sem(aryBold[7],axis=1)
plt.errorbar(xaxis,np.mean(aryBold[5],axis=1), yerr=yerr1, label='both limits (default)')
plt.errorbar(xaxis,np.mean(aryBold[7],axis=1), yerr=yerr2, label='both limits (default)')
plt.legend(['Stimulus Event','Lum_Incre','Lum_Decre'],loc=1)
plt.xticks(ind)
plt.ylim([-1.5,1])
plt.xlabel('Time/volume, TR = 2.604')
plt.ylabel('% BOLD Signal Change')
plt.title('Stimulus_Center['+str(lstTxt[0].shape[1])  +'voxels]_BOLD_Signal_Change')
plt.savefig('Stimulus_Center['+str(lstTxt[0].shape[1])  +'voxels]_BOLD_Signal_Change.svg')
plt.savefig('Stimulus_Center['+str(lstTxt[0].shape[1])  +'voxels]_BOLD_Signal_Change.png')
...
plt.figure(counter)
plt.hlines(0,0,5,linestyles="dashed")
yerr1 = stats.sem(aryBold[1],axis=1)
yerr2 = stats.sem(aryBold[3],axis=1)
plt.errorbar(xaxis,np.mean(aryBold[1],axis=1), yerr=yerr1, label='both limits (default)')
plt.errorbar(xaxis,np.mean(aryBold[3],axis=1), yerr=yerr2, label='both limits (default)')
plt.ylim([-1.5,1])
plt.legend(['Stimulus Event','Lum_Incre','Lum_Decre'],loc=1)
plt.xticks(ind)
plt.xlabel('Time/volume, TR = 2.604')
plt.ylabel('% BOLD Signal Change')
plt.title('Stimulus_Background['+str(lstTxt[1].shape[1] ) +'voxels]_BOLD_Signal_Change')
plt.savefig('Stimulus_Background['+str(lstTxt[1].shape[1])  +'voxels]_BOLD_Signal_Change.svg')
plt.savefig('Stimulus_Background['+str(lstTxt[1].shape[1])  +'voxels]_BOLD_Signal_Change.png')

...
strPathOut = '/media/h/P04/Data/BIDS/sub-02/ses-002/func/GLM/BI/ERA_01'
os.chdir(strPathOut)
...
for index_01 in range(0, varNumIn_01):
    ...
    mne_tc = np.mean(aryTmp,axis = 1)
    mne_tc_mne = np.mean(mne_tc)
    mne_tc_std = np.std(mne_tc)
    mne_tc_norm = np.divide(np.subtract(mne_tc,mne_tc_mne),mne_tc_std)
    lstTxt_mean[index_01] = mne_tc
    lstTxt_norm[index_01] = mne_tc_norm
...
for runid in range(0,varNumIn_02):
     ...
     currplt_bkgrd[:,7],currplt_ctr[:,7] =  lstTxt_norm[2*runid], lstTxt_norm[(2*runid)+1]
     counter+=1
     ...
     plt.plot(lstTxt_norm[(2*runid)+1],c='grey')
...
for index_01 in range(0, len(stiROI)):
     ...
     for index_02 in range(0, varNumIn_02):
          ...
          varCCond_tc = lstTxt_mean[varCindex]
          ...
          varStr_up = np.floor(np.add(np.divide(varCEV_up[:,0],varTR),tplBase[0])).astype(int)
          varStp_up = np.ceil(varStr_up + np.absolute(tplBase[0]) + varDur + varVolsPst).astype(int)

          varStr_down = np.floor(np.add(np.divide(varCEV_down[:,0],varTR),tplBase[0])).astype(int)
          varStp_down = np.ceil(varStr_down + np.absolute(tplBase[0]) + varDur + varVolsPst).astype(int)
          
          varRestTme = np.mean(varCCond_tc[varVolmov:15])
                         

          
          # Generate epock
          varBse_up = np.zeros([BaseDur,varTmpNumBlck])
          varBse_up_perc = np.zeros([BaseDur,varTmpNumBlck])
          varBse_up_perc_01 = np.zeros([BaseDur,varTmpNumBlck])


          varBse_down = np.zeros([BaseDur,varTmpNumBlck])
          varBse_down_perc = np.zeros([BaseDur,varTmpNumBlck])
          varBse_down_perc_01 = np.zeros([BaseDur,varTmpNumBlck])

          counter +=1
          for index_03 in range(0,varTmpNumBlck):
               # Get every epoch
               varBse_up[:,index_03] = varCCond_tc[varStr_up[index_03]:varStp_up[index_03]]
               varBse_down[:,index_03] = varCCond_tc[varStr_down[index_03]:varStp_down[index_03]]

               # Calculate baseline before every epoch
               tmpBse_up = np.mean(varCCond_tc[varStr_up[index_03]:varStr_up[index_03]+2])
               tmpBse_down = np.mean(varCCond_tc[varStr_down[index_03]:varStr_down[index_03]+2])
               
               # Global and local baseline                
               glbbse_up= np.divide((varRestTme + tmpBse_up),2)
               glbbse_down= np.divide((varRestTme + tmpBse_down),2)

               varBse_up_perc[:,index_03] = (np.divide(varBse_up[:,index_03],tmpBse_up)-1) * 100
               varBse_up_perc_01[:,index_03] = (np.divide(varBse_up[:,index_03],glbbse_up)-1) * 100
#               varBse_up_perc[:,index_03] = (np.divide(varBse_up[:,index_03],tmpBse_up)-1) 
#               varBse_up_perc_01[:,index_03] = (np.divide(varBse_up[:,index_03],glbbse_up)-1)


               varBse_down_perc[:,index_03] = (np.divide(varBse_down[:,index_03],tmpBse_down)-1) * 100
               varBse_down_perc_01[:,index_03] = (np.divide(varBse_down[:,index_03],tmpBse_down)-1) * 100

#               varBse_down_perc[:,index_03] = (np.divide(varBse_down[:,index_03],tmpBse_down)-1)
#               varBse_down_perc_01[:,index_03] = (np.divide(varBse_down[:,index_03],tmpBse_down)-1)

               varBOLD_perc_up[:,index_02*varTmpNumBlck:(index_02+1)*varTmpNumBlck]  =    varBse_up_perc
               varBOLD_perc_up_01[:,index_02*varTmpNumBlck:(index_02+1)*varTmpNumBlck]  =    varBse_up_perc_01
               
               varBOLD_perc_down[:,index_02*varTmpNumBlck:(index_02+1)*varTmpNumBlck]  =    varBse_down_perc
          ...
          counter +=1
          plt.figure(counter)
          plt.plot(xaxis,np.mean(varBse_up_perc,axis=1))
          plt.hlines(0,0,5,linestyles="dashed")
          plt.xticks(ind)
          plt.xlabel('Time/volume, TR = 2.604')
          plt.ylabel('% BOLD Signal Change')
          ...
          counter +=1
          plt.figure(counter)
          plt.plot(xaxis,varBse_down_perc)
          plt.hlines(0,0,5,linestyles="dashed")
          plt.xticks(ind)
          plt.legend(['Block1','Block2','Block3','Block4','Block5','Block6','Stimulus Dur'])
          ...
          counter +=1
          plt.figure(counter)
          plt.plot(xaxis,np.mean(varBse_down_perc,axis=1))
          plt.hlines(0,0,5,linestyles="dashed")
          plt.xticks(ind)
          plt.xlabel('Time/volume, TR = 2.604')
          plt.ylabel('% BOLD Signal Change')
          ...
          varBOLD_perc_up_mne[:,index_02]  =    np.mean(varBse_up_perc,axis=1)
          varBOLD_perc_down_mne[:,index_02]  =    np.mean(varBse_down_perc,axis=1)
...
yerr2 = stats.sem(aryBold[4],axis=1)
plt.errorbar(xaxis,np.mean(aryBold[0],axis=1), yerr=yerr1, label='both limits (default)')
plt.errorbar(xaxis,np.mean(aryBold[4],axis=1), yerr=yerr2, label='both limits (default)')
...
yerr1 = stats.sem(aryBold[8],axis=1)
yerr2 = stats.sem(aryBold[12],axis=1)
plt.errorbar(xaxis,np.mean(aryBold[8],axis=1), yerr=yerr1, label='both limits (default)')
plt.errorbar(xaxis,np.mean(aryBold[12],axis=1), yerr=yerr2, label='both limits (default)')
...
#          # find the index in the orginal text file lists
#          varCindex = AllTxtFiles.index(varCCond)
#          varCCond_tc = lstTxt_norm[varCindex]
#          
#          currDM_up = lstEV_gw
#          currDM_down = lstEV_gb
#          
...
#          xaxis = np.arange(-2,17)     
#          plt.figure(counter)
#          plt.plot(xaxis,varBse_up)
#          plt.hlines(0,0,5,linestyles="dashed")
#          plt.xticks(ind)
#          plt.xlabel('Time/volume, TR = 2.604')
#          plt.legend(['Block1','Block2','Block3','Block4','Block5','Block6','Stimulus Dur'])
#
#          plt.ylabel('Normalized BOLD Signal')
#          plt.title( 'fun_0' + str((index_02+1)) + '_' +  \
#                    stiROI[index_01] + '_up_Nomalized_BOLD_Each_Block')
#          plt.savefig( 'fun_0' + str((index_02+1)) + '_' +  \
#                    stiROI[index_01] + '_up_Nomalized_BOLD_Each_Block.svg')
#          plt.savefig( 'fun_0' + str((index_02+1)) + '_' +  \
#                    stiROI[index_01]+ '_up_Nomalized_BOLD_Each_Block.png')
#
#
#
#          counter +=1
#          plt.figure(counter)
#          plt.plot(xaxis,np.mean(varBse_up,axis=1))
#          plt.hlines(0,0,5,linestyles="dashed")
#          plt.xticks(ind)
#          plt.xlabel('Time/volume, TR = 2.604')
#          plt.ylabel('Normalized BOLD Signal')
#          plt.title( 'fun_0' + str((index_02+1)) + '_' +  \
#                    stiROI[index_01]+ '_up_Nomalized_BOLD')
#          plt.savefig( 'fun_0' + str((index_02+1)) + '_' +  \
#                    stiROI[index_01] + '_up_Nomalized_BOLD.svg')
#          plt.savefig( 'fun_0' + str((index_02+1)) + '_' +  \
#                    stiROI[index_01] + '_up_Nomalized_BOLD.png')
#
#          
#          
#          #-----------------------------------------------------------------
...
#
#          plt.title( 'fun_0' + str((index_02+1)) + '_' +  \
#                    stiROI[index_01] + '_down_Nomalized_BOLD_Each_Block')
#          plt.savefig( 'fun_0' + str((index_02+1)) + '_' +  \
#                    stiROI[index_01] + '_down_Nomalized_BOLD_Each_Block.svg')
#          plt.savefig( 'fun_0' + str((index_02+1)) + '_' +  \
#                    stiROI[index_01]+ '_down_Nomalized_BOLD_Each_Block.png')
#
#
#
#          counter +=1
#          plt.figure(counter)
#          plt.plot(xaxis,np.mean(varBse_down,axis=1))
#          plt.hlines(0,0,5,linestyles="dashed")
#          plt.xticks(ind)
#          plt.xlabel('Time/volume, TR = 2.604')
#          plt.ylabel('Normalized BOLD Signal')
#          plt.title( 'fun_0' + str((index_02+1)) + '_' +  \
#                    stiROI[index_01]+ '_down__Nomalized_BOLD')
#          plt.savefig( 'fun_0' + str((index_02+1)) + '_' +  \
#                    stiROI[index_01] + '_down_Nomalized_BOLD.svg')
#          plt.savefig( 'fun_0' + str((index_02+1)) + '_' +  \
#                    stiROI[index_01] + '_down_Nomalized_BOLD.png')
#
#     
#
#plt.close('all')

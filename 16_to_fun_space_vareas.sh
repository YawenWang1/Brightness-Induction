...
mskPth='/media/g/P04/Data/BIDS/sub-12/ses-002/func/GLM/Anat_space/Masks/'
mkdir ${mskPth}
mkdir ${mskPth}/fun_vareas

moving_fun=${funPth}sub-12_ses-002_task-BI_acq-EP3D_dir-RL_run-1_echo-1_bold_fixed_dc_template0.nii.gz
fun_msk=${funPth}sub-12_ses-002_task-BI_acq-EP3D_dir-RL_run-1_echo-1_bold_fixedMask.nii.gz
...
mskFiles=( $(ls | grep sub-12 | grep .nii.gz) )
numMsk=${#mskFiles[@]}
numMsk=$(($numMsk - 1))
echo ${numMsk}
# -----------------------------------------------------------------------------
...
do
  ...
  -i ${mskPth}${mskFiles[${i}]} \
  -o ${mskPth}fun_vareas/${interp01}_reg2run.nii.gz \
  -n GenericLabel \
  -r ${moving_fun} \
  ...
  fslmaths ${mskPth}fun_vareas/${interp01}_reg2run.nii.gz -mul ${fun_msk} \
  ${mskPth}fun_vareas/${interp01}_reg2run_msk.nii.gz
done

...
-o ${Outpth}pRF_results_ovrlp_mask_90prct_${prf_roi_namearray[${i}]}_reg2fsanat.nii.gz \
-n GenericLabel \
-r ${fixed_anat} \
...
$ANTSPATH/antsApplyTransforms -d 3 \
-i ${prfPth}stimuli_roi_ori.nii.gz \
-o ${Outpth}stimuli_roi_ori_reg2fsanat.nii.gz \
-n GenericLabel \
-r ${fixed_anat} \
-t ${trfPth}sub-02_ses-001_prf_to_fs_0GenericAffine.mat \
-v
...
for hemi in {"lh","rh"}; do
  ...
  mri_vol2surf \
  --mov ${Outpth}stimuli_roi_ori_reg2fsanat.nii.gz \
  --regheader ${subjID} \
  --hemi ${hemi} \
  --o ${prfsrfOptPth}stimuli_roi_ori_reg2fsanat_${hemi}.nii.gz \
  --interp nearest
done

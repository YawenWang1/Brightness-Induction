...
do
  ...
  dcmatTrf="${dctrfFilesPth}fun_01_dc_fun_01_template000GenericAffine.mat"

  echo ${dcmatTrf}
  # dcwarp="${dctrfFilesPth}fun_0${runid}_dc_fun_0${runid}_template001Warp.nii.gz"
  dcwarp="${dctrfFilesPth}fun_01_dc_fun_01_template001Warp.nii.gz"

  echo ${dcwarp}
  # ana2dcTxt="${dcanaFilePth}fun_0${runid}_ana_to_dc.txt"
  # echo ${dc2anaTxt}
  curr4dfun="${OutPth}fun_0${runid}_MDC_02.nii.gz"
  ...
  do
    ...
    echo ${mocoTrf}
    currOut="${currOutFolder}fun_0${runid}_vol_${volid}_MC_DC_02.nii.gz"
    ...
    fslmaths \
    ${currOut} \
    ${currOutFolder}fun_0${runid}_vol_${volid}_MC_DC_srt_02.nii.gz \
    -odt short
  ...
  2.604 \
  0 \
  ${currOutFolder}/*_MC_DC_srt_02.nii.gz
...
do
  funPth="${mocoTrfPthUp}run_0${runid}_subset/"
  currOutFolder="${OutPth}run_0${runid}_subset_MC_DC/"
  mkdir ${currOutFolder}
  # dcmatTrf="${dctrfFilesPth}fun_0${runid}_dc_fun_0${runid}_template000GenericAffine.mat"
  dcmatTrf="${dctrfFilesPth}fun_01_dc_fun_01_template000GenericAffine.mat"
  ...
  do
    ...
    mocoTrf="${funPth}fun_0${runid}_vol_${volid}_0GenericAffine.mat"
    echo ${mocoTrf}
    currOut="${currOutFolder}fun_0${runid}_vol_${volid}_MC_DC.nii.gz"
    $ANTSPATH/antsApplyTransforms \
    -d 3 \
    --float \
    -i ${currIn} \
  ...
  2.604 \
  0 \
  ${currOutFolder}/*_MC_DC_srt_.nii.gz

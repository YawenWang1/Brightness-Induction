...
BI_data_path = '/media/h/P04/Data/BIDS/sub-02/ses-001/func/GLM/pRF/pRF_results'

# Path of nii file with x-coordinates of pRFs:
strNiiX = (BI_data_path
           + '/results_x_pos.nii.gz')

# Path of nii file with y-coordinates of pRFs:
strNiiY = (BI_data_path
           + '/results_y_pos.nii.gz')

# Path of nii file with pRF size estimates (SD):
strNiiSd = (BI_data_path
            + '/results_SD.nii.gz')

# Path of nii file with R2 values:
strNiiR2 = (BI_data_path
            + '/results_R2.nii.gz')

# Output file base name:
strNiiOt = (BI_data_path
            + '/pRF_results_')
...
lstLimEdgX = [(-6.5, -5.5), (5.5, 6.5)]
lstLimEdgY = [(-3, -2), (2, 3)]

# Limits of background ROI:
lstLimBckX = [(-8.3, -7.0), (7.0, 8.3)]
lstLimBckY = [(-5.19, -3.5), (3.5, 5.19)]
...
def fncPrfOvrlp(idxPrc,
    ...
    aryCentreChnk = np.zeros(varNumVoxChnk)/media/h/P04/Data/BIDS/code/sub-02/14_Reg_4D_fMRI.sh
...
aryLgcBckX = np.logical_or(
                          np.logical_and(
                                         np.greater_equal(arySpaceXcrt,
                                                    lstLimBckX[0][0]),
                                         np.less_equal(arySpaceXcrt,
                                                 lstLimBckX[0][1])
                                         ),
                          np.logical_and(
                                         np.greater_equal(arySpaceXcrt,
...
aryLgcBckY = np.logical_or(np.logical_and(
                                         np.greater_equal(arySpaceYcrt,
                                                    lstLimBckY[0][0]),
                                         np.less_equal(arySpaceYcrt,
                                                 lstLimBckY[0][1])
                                         ),
                          np.logical_and(
                                         np.greater_equal(arySpaceYcrt,
...
aryLgcBck = np.add(aryLgcBckX,aryLgcBckY)
aryLgcBckidx = aryLgcBck > 0
aryLgcBck[aryLgcBckidx] = 1

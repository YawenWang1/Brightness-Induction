...
pRF_oc_ROI=( $(ls | grep pRF_results_ovrlp_mask_${olvp_prct[${olvpid}]}prct | grep _lh.label) )
numROI=${#pRF_oc_ROI[@]}
numROI=$(($numROI - 1))
echo ${numROI}
# -----------------------------------------------------------------------------
# Get all the pRF overlay results
pRF_oc_ROI_rh=( $(ls | grep pRF_results_ovrlp_mask_${olvp_prct[${olvpid}]}prct | grep _rh.label) )
# -----------------------------------------------------------------------------
cd ${strPathOrig}
...
interp_01=$(cut -d '_' -f 5,6 <<< ${pRF_oc_ROI[${i}]})
interp_rh_01=$(cut -d '_' -f 5,6 <<< ${pRF_oc_ROI_rh[${i}]})

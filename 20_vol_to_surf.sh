...
#--------------------------------------------------------------------------------------
##### --metric CC[fixedImage, movingImage......]
#     that's why the antsApplyTransforms has to use to inverse of the transformation matrix 
...
currPth="/media/h/P04/Data/S04/Ses01/03_GLM/SriPlay/OC_ROI_masks_glm_cntr/00_Lum_Up/"
currPthout="/media/h/P04/Data/S04/Ses01/03_GLM/SriPlay/OC_ROI_masks_glm_cntr/03_In_surface/01_Up/"
mkdir ${currPthout}
cd ${currPth}
...
for i in $(seq 0 $numFiles); do
  ...
  --regheader S04 \
  --hemi ${hemi} \
  --o ${currPthout}${hemi}_up_${Files[${i}]} \
  --interp nearest
done
done
...
# for i in {1..7}; doT
# mri_vol2surf \
# --mov 1Run_ss/results_227vol_1run_sm_hpf_${prf_namearray[${i}]}.nii.gz \
# --regheader S04 \
# --hemi ${hemi} \
# --o 1Run_ss/results_227vol_1run_sm_hpf_${prf_namearray[${i}]}_reg2anat_${hemi}.nii.gz \
# --interp nearest
# done
# done

...
ope_Image=${subject_id}_${session_id}_task-pRF_acq-EP3D_dir-LR_run-1_echo-1_bold
...
echo "-----> Making a fixed Image for pRF runs."
echo " "
...
echo ${tmax}
# Crop the first five volums of the 1st pRF run to make the fixed image
...
$ANTSPATH/ImageMath \
4 \
${Fun_ROI_Folder}${pRF_prefix}${FixRunID}_echo-1_bold_.nii.gz \
...
for volume in $(eval echo "{$fromvol..$nthvol}"); do
    ...
    FIXED=${Fun_ROI_Folder}${pRF_prefix}${FixRunID}_echo-1_bold_${basevol}.nii.gz
    MOVING=${Fun_ROI_Folder}${pRF_prefix}${FixRunID}_echo-1_bold_${volume}.nii.gz
    OUTPUT=${Fun_ROI_Folder}${pRF_prefix}${FixRunID}_echo-1_bold_${volume}
...
$ANTSPATH/ImageMath \
4 \
${fun_Pth}${pRF_prefix}${FixRunID}_echo-1_bold_roi_MoCorr.nii.gz \
TimeSeriesAssemble \
$TR \
0 \
${Fun_ROI_Folder}${pRF_prefix}${FixRunID}_echo-1_bold_${basevol}.nii.gz \
...
3 \
${fun_Pth}${pRF_prefix}${FixRunID}_echo-1_bold_fixed.nii.gz \
2 \
${Fun_ROI_Folder}${pRF_prefix}${FixRunID}_echo-1_bold_${basevol}.nii.gz \
${Fun_ROI_Folder}/*_Warped.nii.gz
...
for i in {1..3}; do
    if [ "$i" -eq "1" ]; then
        ...
        --input-image ${fun_Pth}${pRF_prefix}${FixRunID}_echo-1_bold_fixed.nii.gz \
        --output ${fun_Pth}${pRF_prefix}${FixRunID}_echo-1_bold_fixed_N4.nii.gz
    else
        N4BiasFieldCorrection \
        --image-dimensionality 3 \
        --shrink-factor 2 \
        ...
        --input-image ${fun_Pth}${pRF_prefix}${FixRunID}_echo-1_bold_fixed_N4.nii.gz \
        --output ${fun_Pth}${pRF_prefix}${FixRunID}_echo-1_bold_fixed_N4.nii.gz
    fi
...
--input-image ${fun_Pth}${pRF_prefix}${FixRunID}_echo-1_bold_fixed_N4.nii.gz \
--noise-model Rician \
--output ${fun_Pth}${pRF_prefix}${FixRunID}_echo-1_bold_fixed_N4_DEN.nii.gz

## CREATE ROUGH IMAGE MASK AFTER pRFAS-CORRECTION
...
#
# $ANTSPATH/ImageMath 3 sub-03_ses-001_task-pRF_acq-EP3D_dir-RL_run-1_echo-1_bold_fixedMask_brain.nii.gz ME sub-03_ses-001_task-pRF_acq-EP3D_dir-RL_run-1_echo-1_bold_fixedMask_brain.nii.gz 1
# $ANTSPATH/ImageMath 3 sub-03_ses-001_task-pRF_acq-EP3D_dir-RL_run-1_echo-1_bold_fixedMask_brain.nii.gz MD sub-03_ses-001_task-pRF_acq-EP3D_dir-RL_run-1_echo-1_bold_fixedMask_brain.nii.gz 2
# $ANTSPATH/ImageMath 3 sub-03_ses-001_task-pRF_acq-EP3D_dir-RL_run-1_echo-1_bold_fixedMask_brain.nii.gz MD sub-03_ses-001_task-pRF_acq-EP3D_dir-RL_run-1_echo-1_bold_fixedMask_brain.nii.gz 2
